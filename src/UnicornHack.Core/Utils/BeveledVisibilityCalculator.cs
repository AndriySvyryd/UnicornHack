...
public class BeveledVisibilityCalculator
{
        ...
        GameManager manager)
    {
        ...
        if (targetVector.IsCardinalAligned())
        {
            ...
            return CombineAdjacentOctants(visibleTerrainList, adjacentVisibleTerrainList, origin,
                onDiagonal: false);
        }

        if (targetVector.IsIntercardinalAligned())
        {
            Compute(targetOctant, origin,
                range,
                linearFalloff: 0,
        ...
        bool onDiagonal)
    {
        ...
        while (true)
        {
            var (firstIndex, firstVisibility) =
                m == visibleTerrainList.Count ? (-1, (byte)0) : visibleTerrainList[m];
            var (secondIndex, secondVisibility) = n == adjacentVisibleTerrainList.Count
                ? (-1, (byte)0)
                : adjacentVisibleTerrainList[n];
            ...
            if (firstIndex != -1)
            {
                if ((!((onDiagonal && firstVector.IsIntercardinalAligned()) ||
                       (!onDiagonal && firstVector.IsCardinalAligned()))
            ...
            if (secondIndex != -1)
            {
                if ((!(onDiagonal && secondVector.IsIntercardinalAligned() ||
                       (!onDiagonal && secondVector.IsCardinalAligned()))
        ...
        List<(int, byte)> visibleTerrainList)
    {
        ...
        for (; x <= range; x++)
        {
            ...
            for (var sectorIndex = 0; sectorIndex < sectors.Length; sectorIndex++)
            {
                ...
                {
                    sectors[sectorIndex].NextBottomIntersectionY =
                        ...
                        if (wasOpaque == 0)
                        {
                            ...
                            if (newTop.Greater(bottom))
                            {
                                ...
                                if (y == topY)
                                {
                                    ...
                                    sectors[sectorIndex].NextTopIntersectionY =
                                ...
                                {
                                    Span<(float NextTopIntersectionY, float NextBottomIntersectionY, Slope Top,
                                        Slope Bottom)> newSectors =
                    ...
                    else
                    {
                        ...
                        if (y == bottomY)
                        {
                            ...
                            visibility = (byte)(visibility * visibleArea);
                                 ...
                                 bottomY != (int)(sectors[sectorIndex].NextBottomIntersectionY + 0.5f))
                        {
                            var bottomSideIntersectionY = y - 0.5f;
                            var bottomSideIntersectionX = bottomSideIntersectionY * bottom.X / bottom.Y;
