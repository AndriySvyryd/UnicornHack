...
public static class Extensions
{
    ...
    public static IReadOnlyCollection<T> GetNonRedundantFlags<T>(this T flags, bool removeComposites)
    {
        ...
        foreach (var currentValue in values.ToList())
        {
            ...
            if (decomposedValues.Count > 1)
            {
                ...
                {
                    values.ExceptWith(decomposedValues.Where(v
                        => !EqualityComparer<T>.Default.Equals(v, currentValue)));
                }
            }
        }
    ...
    private class PropertyAccessorExpressionVisitor : ExpressionVisitor
    {
        ...
        protected override Expression VisitMember(MemberExpression node)
        {
            ...
            if (propertyInfo == null)
            {
                throw new InvalidOperationException(node.Member.Name +
                                                    " is not a property, only properties are supported.");
            }
            ...
            getExpression =
                Expression.Condition(condition, Expression.Default(propertyAccess.Type), propertyAccess);
