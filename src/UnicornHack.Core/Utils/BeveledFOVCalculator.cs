...
namespace UnicornHack.Utils
{
    ...
    /// </summary>
    public class BeveledFOVCalculator
    {
        ...
        /// <summary>
        ///     Creates a new instance of <see cref="BeveledFOVCalculator" />
        /// </summary>
        ...
        /// </remarks>
        public BeveledFOVCalculator(Func<byte, byte, DirectionFlags> getUnconnectedNeighbours)
            => _getUnconnectedNeighbours = getUnconnectedNeighbours;

        public void Compute<TState>(Point origin, byte rangeLimit, byte visibilityFalloff,
            Func<byte, byte, byte, TState, bool> blocksVisibility, TState state)
        {
            blocksVisibility(origin.X, origin.Y, MaxVisibility, state);
            for (byte octant = 0; octant < 8; octant++)
            {
                Compute(octant, origin, rangeLimit, visibilityFalloff,
                    x: 1, top: new Slope(1, 1), bottom: new Slope(0, 1),
                    blocksVisibility, state);
            }
        }
        ...
        ///     The function must ignore coordinates that are out of bounds
        /// </param>
        /// <param name="state"> Additional state for <paramref name="blocksVisibility" /></param>
        /// <param name="noFalloff"> Disables visibility falloff is set to <c>true</c></param>
        /// <typeparam name="TState"> Type of the state parameter for <paramref name="blocksVisibility" /></typeparam>
        public void Compute<TState>(
            Point origin, Direction heading,
            int primaryFOVQuadrants, int primaryRange,
            int totalFOVQuadrants, int secondaryRange,
            Func<byte, byte, byte, TState, bool> blocksVisibility,
            TState state,
            bool noFalloff)
        {
            ...
            for (var octant = totalFOVQuadrants - primaryFOVQuadrants;
                ...
                octant++)
            {
                Compute((byte)((octant + octantShift) % 8), origin, primaryRange, visibilityFalloff,
                    x: 1,
                    top: new Slope(1, 1),
                    bottom: new Slope(0, 1),
                    blocksVisibility,
            ...
            TState state)
        {
            ...
            for (; x <= rangeLimit; x++)
            {
                ...
                for (var y = topY; y >= bottomY; y--)
                {
                    ...
                    var lightBlocked = BlocksVisibility(x, y, octant, origin, visibility, blocksVisibility, state);
                        ...
                        if (wasOpaque == 0)
                        {
                            ...
                            if (top.Greater(newBottomY, newBottomX))
                            {
                                ...
                                Compute(octant, origin, rangeLimit, visibilityFalloff, x + 1, top,
                                    new Slope(newBottomY, newBottomX), blocksVisibility, state);
                            }
                            else
                            {
        ...
        private bool BlocksVisibility<TState>(int x, int y, int octant, Point origin, byte visibility,
            Func<byte, byte, byte, TState, bool> blocksVisibility, TState state)
        {
            ...
            return blocksVisibility((byte)nx, (byte)ny, visibility, state);
        }
