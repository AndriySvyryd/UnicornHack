...
namespace UnicornHack.Services.English
{
    public class EnglishMorphologicalProcessor
    {
        ...
        public virtual string ProcessVerbSimplePresent(string verbPhrase, EnglishPerson person)
            => ProcessVerb(verbPhrase, person == EnglishPerson.Third
            ...
            EnglishPronounForm form, EnglishNumber number, EnglishPerson person, EnglishGender? gender)
        {
            ...
            var personIndex = person == EnglishPerson.Third
                ? 2 + (int)gender.Value
                : (int)person;
