        ...
        public void Relationship_is_updated()
        {
            ...
            using (var levelEntityReference = manager.CreateEntity())
            {
                ...
                using (var firstPositionEntityReference = manager.CreateEntity())
                {
                    ...
                    Assert.True(manager.LevelActorsToLevelCellRelationship.Dependents.ContainsEntity(firstPositionEntity.Id));
                    Assert.Same(firstPositionEntity, manager.LevelActorsToLevelCellRelationship.Dependents.FindEntity(firstPositionEntity.Id));
                    Assert.Same(firstPositionEntity,
                        manager.LevelActorsToLevelCellRelationship.GetDependents(levelEntity).GetValueOrDefault(new Point(2, 3)));
                    ...
                    using (var secondLevelEntityReference = manager.CreateEntity())
                    {
                        ...
                        Assert.Null(manager.LevelActorsToLevelCellRelationship.GetDependents(levelEntity).GetValueOrDefault(new Point(1, 2)));
                    ...
                    Assert.False(manager.LevelActorsToLevelCellRelationship.Dependents.ContainsEntity(firstPositionEntity.Id));
