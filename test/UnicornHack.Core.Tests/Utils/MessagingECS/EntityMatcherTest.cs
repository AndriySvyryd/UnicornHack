    ...
    public void Matches()
    {
        ...
        using (var entityReference = manager.CreateEntity())
        {
            ...
            Assert.True(new EntityMatcher<GameEntity>().AllOf((int)EntityComponent.AI).AllOf((int)EntityComponent.Being)
                .Matches(entity));
            Assert.False(new EntityMatcher<GameEntity>().AllOf((int)EntityComponent.AI)
                .AllOf((int)EntityComponent.Player)
                .Matches(entity));

            Assert.True(new EntityMatcher<GameEntity>().AnyOf((int)EntityComponent.AI)
                .AnyOf((int)EntityComponent.Player)
                .Matches(entity));
            Assert.False(new EntityMatcher<GameEntity>().AnyOf((int)EntityComponent.Physical)
                .AnyOf((int)EntityComponent.Player)
                .Matches(entity));

            Assert.True(new EntityMatcher<GameEntity>().NoneOf((int)EntityComponent.Physical)
                .NoneOf((int)EntityComponent.Player)
                .Matches(entity));
            Assert.False(new EntityMatcher<GameEntity>().NoneOf((int)EntityComponent.AI)
                .NoneOf((int)EntityComponent.Player)
                .Matches(entity));
